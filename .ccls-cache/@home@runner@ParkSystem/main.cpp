...
public:
  ...
  // map of registration number and slot

  //age and list of registration number and slot of driver 
  ...
  void parkCar(string regNo , int age) {
    ...
    if(mp1.size() == MAX_SIZE) cout << "Sorry, Parking is full\n";
  ...
  void getSlotsWithRespectToAge(int age) {
    if(mp3.find(age) == mp3.end()) writeInFile("\n");
